...
functional time series to get a quantifiable and reproducible exclusion
criterion (cf. Marquardt et al. 2017; Bergmann et al. 2019). Reference volumes
can be for example the session mean. A regressor of no interest is written for
...
_, name_file, _ = get_filename(file_in[0])

# create output folder
if len(file_in) < 2:
    path_output = os.path.join(os.path.dirname(file_in[0]), "correlation")
else:
    path_output = os.path.join(os.path.dirname(os.path.dirname(file_in[0])),
                               "correlation")
...
for i in range(len(file_in)):
    ...
    # load time series
    data_temp = nb.load(file_in[i]).get_fdata()

    # print progress
    print("Time series " + str(i + 1) + "/" + str(len(file_in)))

    # create output folder for regressor of no interest
    path_logfile = os.path.join(os.path.dirname(file_in[i]), "outlier")
    if not os.path.exists(path_logfile):
        os.makedirs(path_logfile)

    # open logfile for regressor of no interest
    file2 = open(os.path.join(path_logfile,
                              "correlation_regressor_" +
                              name_file + ".txt"), "w")
    ...
    for j in range(np.shape(data_temp)[3]):
        ...
        if j < np.shape(data_temp)[3] - 1 and i < len(file_in):
            data_temp_1 = data_temp[:, :, :, j].copy()
            data_temp_2 = data_temp[:, :, :, j + 1].copy()

        elif j == np.shape(data_temp)[3] - 1 and i < len(file_in) - 1:
            data_temp_1 = data_temp[:, :, :, j].copy()
            data_temp_2 = nb.load(file_in[i + 1]).get_fdata()[:, :, :, 0]
    ...
    file.write(
        "Outlier percentage (pearson to ref): " + str(res_pearson_0) + "\n")
...
fig.savefig(os.path.join(path_output, "r_shapiro_" + name_file + ".png"),
...
ax.set_title("Shapiro-Wilk test for each time step")
fig.savefig(os.path.join(path_output, "p_shapiro_" + name_file + ".png"),
            format='png', bbox_inches='tight')
# plt.show()
...
fig.savefig(os.path.join(path_output, "r_pearson_0_" + name_file + ".png"),
            format='png', bbox_inches='tight')
# plt.show()
...
ax.set_title("Time series spatial correlation to volume ref")
fig.savefig(os.path.join(path_output, "p_pearson_0_" + name_file + ".png"),
            format='png', bbox_inches='tight')
# plt.show()
...
fig.savefig(os.path.join(path_output, "r_pearson_" + name_file + ".png"),
            format='png', bbox_inches='tight')
# plt.show()
...
ax.set_ylabel("p-value (Pearson correlation)")
ax.set_title("Time series spatial correlation to volume i-1")
fig.savefig(os.path.join(path_output, "p_pearson_" + name_file + ".png"),
            format='png', bbox_inches='tight')
# plt.show()
