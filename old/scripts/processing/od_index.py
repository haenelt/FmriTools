...
This scripts calculates a defined ocular dominance (OD) index for a session
consisting of several runs. From the condition file which has to be in the SPM
compatible *.mat format, time points for both experimental conditions are
extracted. Condition time points can be converted to z-scores. The OD index is
computed by dividing each condition mean by the condition mean or max within a
predefined mask before computing the difference of both conditions. The index
for the whole session is taken as the average across single runs. If the outlier
input array is not empty, outlier volumes are discarded from the analysis.
Optionally, the time series can be filtered by a lowpass and a highpass filter.
The input images should be in nifti format. The script needs an installation of
afni.
...
epi2orig_input = (
    "/data/pt_01880/Experiment2_Rivalry/p3/deformation/retinotopy/epi2orig.nii.gz"
)
...
path_output = os.path.join(
    os.path.dirname(os.path.dirname(path_file)), "results", "od", "native"
)
...
apply_coordinate_mappings(
    ...
    file_name="mask"
    # base name with file extension for output
)

# rename final deformations
os.rename(
    os.path.join(path_output, "mask_def-img.nii.gz"),
...
if len(name_output) and len(name_sess):
    ...
    fileOUT1 = os.path.join(
        path_output, "od_" + name_output + "_" + condition1 + "_" + condition2 + ".nii"
    )
    fileOUT2 = os.path.join(
        path_output, "od_" + name_output + "_" + condition2 + "_" + condition1 + ".nii"
    )
elif not len(name_output) and len(name_sess):
    fileOUT1 = os.path.join(
        path_output, "od_" + condition1 + "_" + condition2 + "_" + name_sess + ".nii"
    )
    fileOUT2 = os.path.join(
        path_output, "od_" + condition2 + "_" + condition1 + "_" + name_sess + ".nii"
    )
else:
