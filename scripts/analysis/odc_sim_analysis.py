...
The purpose of the following script is to simulate ocular dominance columns and
...
the maximum Fourier component.
(4) take the maximum power (relative to central peak) as repetition marker?

Further below, not all parameters are needed for this simulation since we only
use the neural map, i.e., no MRI parameters (Nx_mri, Ny_mri), bold parameters
(beta, fwhm_bold, fwhm_noise) or occlusion parameters (a_mask, b_mask,
theta_mask) are needed.
...
# local inputs
from fmri_tools.simulation.odc import odc_2d
...
for i in range(niter):
    ...
    # get odc pattern
    _, neural, _, _, _, _ = odc_2d(Nx_sim, Ny_sim, FOVx, FOVy, Nx_mri, Ny_mri,
                                   rho, delta, epsilon,
                                   theta, alpha, beta, fwhm_bold, fwhm_noise,
                                   a_mask, b_mask,
                                   theta_mask, False)
    ...
    for j in range(len(phi)):
        ...
        k_fft, P_fft, x_fft, y_fft = analyze_fft(neural, FOVx, FOVy, x_temp,
                                                 y_temp)

        # analyze autocorrelation
        fwhm_acorr, d_acorr, P_acorr, x_acorr, y_acorr = analyze_acorr(neural,
                                                                       FOVx,
                                                                       FOVy,
                                                                       x_temp,
                                                                       y_temp)
...
np.savez(os.path.join(path_output, name_output),
         x_fft_0=x_fft_0, x_fft_90=x_fft_90, x_acorr_0=x_acorr_0,
         x_acorr_90=x_acorr_90,
         y_fft_0=y_fft_0, y_fft_90=y_fft_90, y_acorr_0=y_acorr_0,
         y_acorr_90=y_acorr_90,
         k_fft_phi_mean=k_fft_phi_mean, P_fft_phi_mean=P_fft_phi_mean,
         d_acorr_phi_mean=d_acorr_phi_mean, P_acorr_phi_mean=P_acorr_phi_mean,
         fwhm_acorr_phi_mean=fwhm_acorr_phi_mean,
         k_fft_phi_std=k_fft_phi_std, P_fft_phi_std=P_fft_phi_std,
         d_acorr_phi_std=d_acorr_phi_std, P_acorr_phi_std=P_acorr_phi_std,
         fwhm_acorr_phi_std=fwhm_acorr_phi_std)
