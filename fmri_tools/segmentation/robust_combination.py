...
def robust_combination(fileUNI, fileINV1, fileINV2, regularisation,
    ...
    If the value is too big you will start noticing that the image gets a bigger 
    bias field - which will deteriorate the segmentation results. Once you are 
    happy with the value you found for one subject you can use the same for all 
    the following subjects by just calling the function with the same 
    regularisation parameter.
    
    Usually the regularisation factor shouldn't be greater than 10, but that is 
    not the case when the image is bias field corrected, in which case the noise 
    estimates at the edge of the image <noiselevel> might not be such a good 
    measure.
    ...
    INV1pos_array = rootsquares_pos(-MP2RAGE_array, INV2_array,
                                    -INV2_array ** 2 * MP2RAGE_array)
    INV1neg_array = rootsquares_neg(-MP2RAGE_array, INV2_array,
                                    -INV2_array ** 2 * MP2RAGE_array)
    ...
    INV1final_array[np.abs(INV1_array - INV1pos_array) > np.abs(
        INV1_array - INV1neg_array)] = INV1neg_array[
        np.abs(INV1_array - INV1pos_array) > np.abs(INV1_array - INV1neg_array)]
    INV1final_array[np.abs(INV1_array - INV1pos_array) <= np.abs(
        INV1_array - INV1neg_array)] = INV1pos_array[
        np.abs(INV1_array - INV1pos_array) <= np.abs(
            INV1_array - INV1neg_array)]

    # lambda calculation
    noiselevel = regularisation * np.mean(INV2_array[:, -10:, -10:])
    MP2RAGERobustPhaseSensitive_array = MP2RAGErobustfunc(INV1final_array,
                                                          INV2_array,
                                                          noiselevel ** 2)
    ...
    output = nb.Nifti1Image(MP2RAGERobustPhaseSensitive_array,
                            MP2RAGE_img.affine, MP2RAGE_img.header)
