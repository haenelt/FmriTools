...
def cut_mesh(vertex, x_min=None, x_max=None, y_min=None, y_max=None, z_min=None,
             z_max=None, merge="union"):
...
label = cut_mesh(vtx0, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max,
                 z_min=z_min, z_max=z_max, merge=merge)
...
smooth_surface(file_out0, file_out0, 5)
smooth_surface(file_out1, file_out1, 5)
...
while j < nlayer:
    for i in range(np.shape(ind_array)[1]):
        ...
        line_curr = np.linspace((0, 0, 0), (1, 1, 1,), nlayer + 1, dtype=np.float)
...
smooth_surface(file_out_merge3,
               file_out_merge4,
               n_smooth)

# new overlay
arr_overlay = np.zeros(len(vtx0_new))  # inner side
arr_overlay = np.concatenate((arr_overlay,
                              nb.load(file_contrast[int(nlayer / 2)]).get_fdata()[
                                  ind_keep, 0, 0]))  # outer side
