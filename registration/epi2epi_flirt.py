...
between different epi time series. The script consists of the following steps:
    ...
    4. mask epi
    5. flirt
    6. get deformation field
    7. apply deformation field
...
flirt.inputs.out_matrix_file = os.path.join(path_flirt, "flirt_matrix.txt")
flirt.run()
...
"""
invt = ConvertXFM()
invt.inputs.in_file = os.path.join(path_flirt, "flirt_matrix.txt")
invt.inputs.invert_xfm = True
invt.inputs.out_file = os.path.join(path_flirt, "flirt_inv_matrix.txt")

"""
get cmap
"""
generate_coordinate_mapping(os.path.join(path_epi_target, "pbepi.nii"), 
                            pad = 0, 
...
generate_coordinate_mapping(os.path.join(path_epi_source, "pbepi.nii"), 
                            pad = 0, 
                            path_output = path_flirt, 
                            suffix = "source", 
                            time = False, 
...
applyxfm.inputs.reference = os.path.join(path_epi_source, "pbepi.nii")
applyxfm.inputs.in_matrix_file = os.path.join(path_flirt, "flirt_matrix.txt")
...
applyxfm.inputs.reference = os.path.join(path_epi_target, "pbepi.nii")
applyxfm.inputs.in_matrix_file = os.path.join(path_flirt, "flirt_inv_matrix.txt")
applyxfm.inputs.interp = "trilinear"
applyxfm.inputs.padding_size = 0
applyxfm.inputs.output_type = "NIFTI_GZ"
