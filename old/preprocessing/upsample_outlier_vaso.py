...
# input
file_in = [
    "/data/pt_01880/Experiment1_ODC/p4/odc/VASO1/Run_3/outlier/outlier_regressor_merge.txt"
]
...
for f in range(len(file_in)):
    # set output path and filename
    path_output, _, _ = get_filename(file_in[f])
    name_output = "outlier_regressor_upsampled.txt"

    # load outlier textfile
    outlier = np.loadtxt(file_in[f])
    ...
    for i in range(len(t_new)):
        for j in range(len(t_outlier)):
            if t_outlier[j] - TR_new < t_new[i] < t_outlier[j] + TR_old:
                outlier_new[i] = 1
