...
for i in range(len(path_t1)):
    if ext_mask == ".mgh":
        ...
        mgh2nii(os.path.join(path_t1[i], "mask.mgh"),
                path_t1[i], out_type="nii")
    elif ext_mask == ".mgz":
        sh.copyfile(file_mask, os.path.join(path_t1[i], "mask.mgz"))
        mgh2nii(os.path.join(path_t1[i], "mask.mgz"),
                path_t1[i], out_type="nii")
...
# clean ana
for i in range(len(path_t1)):
    clean_ana(os.path.join(path_t1[i], "T1.nii"), 1000.0, 4095.0,
              overwrite=True)
