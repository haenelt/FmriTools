def shift_white(path, sub, w_shift=-0.5):
    """
    This function shifts the white matter surface inwards (if negative values are applied). This 
    step is included since the white matter surface is biased and lies somewhat within grey matter 
    for MP2RAGE segmentations. A shift of -0.5 mm seems to be a reasonable value. After applying the 
    shift, some measurements are updated including hemi.inflated, hemi.sulc and hemi.curv. Old files 
    are moved to the freesurfer trash folder tagged with a date string as suffix. The curvature file 
    is computed as mean curvature (not Gaussian curvature).
    Inputs:
    ...
    import datetime
    import shutil
    from nipype.interfaces.freesurfer import MRIsExpand, MRIsInflate, Curvature

    # parameters 
    hemi = ["lh","rh"] # hemisphere prefix
    n_inflate = 50 # number of iterations for surface inflation
    curv_distance = (10,10) # number of neighbours within distance in mm for curvature estimate
    ...
    for i in range(len(hemi)):
        for j in range(len(filename)):
            ...
            file_out = os.path.join(path_trash,hemi[i]+"."+filename[j]+"_backup_"+date)
            os.rename(file_in, file_out)

    # shift white matter surface inwards
    for i in range(len(hemi)):
        mris_expand = MRIsExpand()
        mris_expand.inputs.in_file = os.path.join(path_trash,hemi[i]+".white_backup_"+date)
        mris_expand.inputs.thickness = False
        mris_expand.inputs.distance = w_shift
        mris_expand.inputs.out_name = os.path.join(path,sub,"surf",hemi[i]+".white")
        mris_expand.run()
