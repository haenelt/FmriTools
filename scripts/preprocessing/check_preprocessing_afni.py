...
visualization, a regressor of no interest and a short summary are written. The
script needs an installation of fsl and afni.
...
for i in range(len(input_epi)):
    ...
    mask_epi(file_epi_mean, os.path.join(path_temp, "pT1" + ext_t1), file_mask,
    ...
    fig.savefig(os.path.join(path_output, "outlier_afni.png"), format='png',
                bbox_inches='tight')

    # make regressor of no interest
    log_outlier = np.zeros_like(log_data)
    log_outlier[log_data > pthr] = 1
    np.savetxt(os.path.join(path_output,
                            "outlier_afni_regressor_" + name_epi + ".txt"),
               log_outlier, fmt="%d")
