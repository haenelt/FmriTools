...
# parameters
SCALE = 1  # scale parameter for vertex shifts
...
def cut_mesh(
    ...
    """Cut mesh and show layer profile for visualization.
    ...
    file_contrast : list
        List of mgh overlays containing contrasts for each layer.
    path_output : str
        Directory where output is written.
    x_min : int, optional
        Minimum vertex coordinate in x-direction, by default None.
    x_max : int, optional
        Maximum vertex coordinate in x-direction, by default None.
    y_min : int, optional
        Minimum vertex coordinate in y-direction, by default None.
    y_max : int, optional
        Maximum vertex coordinate in y-direction, by default None.
    z_min : int, optional
        Minimum vertex coordinate in z-direction, by default None.
    z_max : int, optional
        Maximum vertex coordinate in z-direction, by default None.
    file_label : _type_, optional
        Optional label file, by default None.
    file_in2 : _type_, optional
        Optional second mesh, by default None.
    merge : str optional
        How to merge second with second mesh (union or intersection), by default
        "union".
    """
    # make output folder
    ...
    while True:
        ...
        if nn_2d_found:
            ...
            hole.append(c)
            nn, _ = _nn_3d(vtx0_new[ind_edge_sorted[-1]], vtx0_new, 100)
            nn_2d_found = True
    ...
    fac_merge = np.concatenate(
        (np.flip(fac_new, axis=1), fac_new + len(vtx0_new)), axis=0
    )
    ...
    for i, _ in enumerate(hole):
        ...
        nn, _ = _nn_3d(vtx0_new[indd], vtx0_new, 10)
        nn = nn[nn != indd]
    ...
    arr_overlay = np.concatenate(
        (
            arr_overlay,
            nb.load(file_contrast[int(nlayer / 2)]).get_fdata()[ind_keep, 0, 0],
        )
    )  # outer side
...
def _remove_face(face, index_keep, index_remove):
...
def _get_edge(index_keep, vtx, fac):
