...
def calculate_distortion(file_patch, file_white, path_output, hemi):
    ...
    for i in range(len(fac_white)):
        if np.any(fac_white[i, 0] == ind_patch) and np.any(
                fac_white[i, 1] == ind_patch) and np.any(
                fac_white[i, 2] == ind_patch):
    ...
    facvtx_white = np.concatenate(
        [vtx_white[fac_patch[:, 0]],
         ...
         vtx_white[fac_patch[:, 2]]], axis=1)
    facvtx0_white = facvtx_white[:, 0:6] - np.concatenate(
        [facvtx_white[:, 6:9],
         facvtx_white[:, 6:9]], axis=1)  # place 3rd vtx at origin
    cp = np.cross(facvtx0_white[:, 0:3],
                  facvtx0_white[:, 3:6], axisa=1, axisb=1)  # cross product
    ...
    facvtx_patch = np.concatenate(
        [vtx_patch_all[fac_patch[:, 0]],
         ...
         vtx_patch_all[fac_patch[:, 2]]], axis=1)
    facvtx0_patch = facvtx_patch[:, 0:6] - np.concatenate(
        [facvtx_patch[:, 6:9],
         facvtx_patch[:, 6:9]], axis=1)  # place 3rd vtx at origin
    cp = np.cross(facvtx0_patch[:, 0:3],
                  facvtx0_patch[:, 3:6], axisa=1, axisb=1)  # cross product
    ...
    write_morph_data(os.path.join(path_output, os.path.basename(
        file_patch) + ".areal_distortion"), VAD)
    ...
    # save morphological data
    write_morph_data(os.path.join(path_output, os.path.basename(
        file_patch) + ".line_distortion"), VLD)
