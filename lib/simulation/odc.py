def odc_2d(Nx_sim=1024, Ny_sim=1024, FOVx=20, FOVy=20, Nx_mri=100, Ny_mri=100, rho=0.5, delta=0.3, 
    ...
    default values for the columnar pattern are taken from this publication. First, a white noise 
    pattern is defined and an anisotropic band-pass filter and a non-linear sigmoidal filter are 
    applied. This neural map is converted to a BOLD response map by applying a Gaussian filter. 
    White noise is added to simulate uncorrelated measurement noise to the signal. The MRI sampling 
    procedure is considered by only inner k-space lines of the spatial frequency representation of 
    the BOLD reseponse map. 
    Inputs:
