...
def shift_white(path, sub, w_shift=-0.5):
    ...
    for i in range(len(hemi)):
        ...
        mris_expand.inputs.out_name = os.path.join(path, sub, "surf",
                                                   hemi[i] + ".white")
        mris_expand.run()
    ...
    for i in range(len(hemi)):
        inflate = MRIsInflate()
        inflate.inputs.in_file = os.path.join(path, sub, "surf",
                                              hemi[i] + ".white")
        inflate.inputs.args = "-n " + str(n_inflate)
        inflate.inputs.out_file = os.path.join(path, sub, "surf",
                                               hemi[i] + ".inflated")
        inflate.inputs.out_sulc = os.path.join(path, sub, "surf",
                                               hemi[i] + ".sulc")
        inflate.run()
