...
for i in range(np.shape(phase)[0]):
    for j in range(np.shape(phase)[1]):
        ...
        if np.size(line[0][0]) == np.size(line[0]):
            ...
            if np.any(np.abs(np.concatenate(
                    (delta_x, delta_y, delta_z))) > max_distance):
                continue
...
nb.save(output, os.path.join(path_output,
                             hemi + "." + name_patch + ".iso_distance.nii"))
...
fig.savefig(
    os.path.join(path_output, hemi + "." + name_patch + ".isolines.png"),
    dpi=400)
plt.close('all')
