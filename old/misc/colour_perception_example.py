...
With this script, the low resolution of colour perception can be shown. An rgb
image is read and converted to greyscale. Then, a grid is defined on the image
and only the grid points are coloured by a smoothed version of the initial
colour image.
...
file_in = "/home/daniel/Bilder/background.jpg"
...
img = io.imread(file_in).astype(float)

# generate greyscale image
img_bw = np.sqrt(img[:, :, 0] ** 2 + img[:, :, 1] ** 2 + img[:, :, 2] ** 2).astype(
    float
)
