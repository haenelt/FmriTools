...
This script looks for outliers in a functional time series using the Afni function 3dToutcount. The
fraction of outliers found within a volume defined by a mask are written out for each time point.
Additionally, a graphical visualization, a regressor of no interest and a short summary are written.

Before running the script, login to queen via ssh and set the fsl and afni environments by calling 
FSL and AFNI in the terminal.
...
from lib.utils.get_mean import get_mean
...
for i in range(len(input_epi)):
    ...
    mask_ana(file_t1, file_mask) # mask t1
