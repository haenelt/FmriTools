...
def get_percentile(input_data, input_cmap, input_distance, min_percentile,
                   max_percentile, min_distance, max_distance):
    ...
    cmap = cmap[np.all([data > np.percentile(data, min_percentile),
                        data < np.percentile(data, max_percentile)], axis=0)]
...
for i in range(len(hemi)):
    ...
    for j in multipol:
        ...
        c = get_percentile(input_data, input_cmap, input_distance,
                           min_percentile, max_percentile, min_distance,
                           max_distance)
        cmap_layer.append(c)
...
for i in range(len(hemi)):
    ...
    for j in range(len(cmap[i])):
        distance_temp2[j] = distance_temp[
            np.where(cmap_temp == cmap[i][j])[0][0]]
...
for k in layer:
    ...
    for i in multipol:
        for j in range(len(hemi)):
            input_data = os.path.join(path_data, hemi[j] + ".c_multipol_" + str(
                i) + "_def-img_layer" + str(k) + "_def.mgh")
            data = nb.load(input_data).get_fdata()
    ...
    if gaussian:
        ...
        odr = odr.ODR(data, gauss_model,
                      beta0=[0, 0.1, 0.5])  # set up ODR with the model and data
    else:
    ...
    plt.errorbar(F_bin_mean, M_bin_mean, yerr=M_bin_std, xerr=F_bin_std,
                 hold=True, ecolor="k", fmt="none", label="data", elinewidth=1)
    ...
    ax.fill_between(F_bin_fit, M_bin_fit_up, M_bin_fit_dw, alpha=.25,
