...
input_phase = (
    "/data/pt_01880/V2STRIPES/p6/retinotopy/avg/surf/rh.ecc_phase_avg_def_layer5.mgh"
)
hemi = "rh"
name_patch = "occip4"
...
def gaussian_filter_edge(u, sigma):
    """This function calculates a Gaussian filter considering the voxels set to
    NaN ouside the grid.
    """
...
for i in range(np.shape(phase)[0]):
    for j in range(np.shape(phase)[1]):
        ...
        if np.size(line[0][0]) == np.size(line[0]):
            ...
            # wrong cmap at the edges indices may lead to unrealistic lengths
            # between neighbors which are discardeds
            if np.any(
                np.abs(np.concatenate((delta_x, delta_y, delta_z))) > max_distance
            ):
                continue
...
nb.save(
    output, os.path.join(path_output, hemi + "." + name_patch + ".iso_distance.nii")
)
...
fig.savefig(
    os.path.join(path_output, hemi + "." + name_patch + ".isolines.png"), dpi=400
)
plt.close("all")
