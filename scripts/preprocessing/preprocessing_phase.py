...
for i in range(np.shape(phase_array)[3]):
    ...
    temp_in = nb.Nifti1Image(phase_array[:, :, :, i], phase_img.affine,
                             phase_img.header)
    temp_out = phase_unwrapping(temp_in, mask=None, nquadrants=3,
...
# remove outliers
t = np.loadtxt(os.path.join(path_magn, "outlier",
                            "outlier_regressor_" + name_magn + ".txt")).astype(int)
...
get_mean(os.path.join(path_magn, name_magn_temp + ext_magn), path_magn,
         name_magn, method="mean")
get_mean(os.path.join(path_phase, name_phase_temp + ext_phase), path_phase,
         name_phase, method="mean")

# phase variance over time
get_std(os.path.join(path_phase, name_phase_temp + ext_phase), path_phase,
        name_phase)
...
phase_array[phase_array > phase_max * np.max(phase_array)] = phase_max * np.max(
    phase_array)
phase_array[
    phase_array < -phase_max * np.max(phase_array)] = -phase_max * np.max(
    phase_array)
...
output = nb.Nifti1Image(magn_array, magn_img.affine, magn_img.header)
nb.save(output,
        os.path.join(path_magn, "mean_" + name_magn + "_enhanced" + ext_magn))
...
# outlier percentage
outlier = np.loadtxt(os.path.join(path_magn, "outlier",
                                  "outlier_regressor_" + name_magn + ".txt"))
